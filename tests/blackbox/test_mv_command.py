...
class TestMvCommand:
    ...
    async def test_metadata_directive_copy(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[0], "HeadObject", Bucket="bucket", Key="key.txt"
        )
        assert_s3_operation(
            server.requests[1],
            "CopyObject",
        ...
        assert_s3_operation(
            server.requests[2], "DeleteObject", Bucket="bucket", Key="key.txt"
        )
    ...
    async def test_no_metadata_directive_for_non_copy(self, aws_cli, tmp_path):
        ...
        assert len(server.requests) == 1, format_requests(server)
        assert_s3_operation(
            server.requests[0], "PutObject", Bucket="bucket", Key="key.txt"
        )
    ...
    async def test_with_copy_props(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[5], "PutObjectTagging", Bucket="bucket", Key="key"
        )
        assert_s3_operation(
            server.requests[6],
            "DeleteObject",
    ...
    async def test_mv_does_not_delete_source_on_failed_put_tagging(
        ...
        assert_s3_operation(
            server.requests[6], "DeleteObject", Bucket="bucket", Key="key"
        )
    ...
    async def test_download_with_checksum_mode_crc32(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[2], "DeleteObject", Bucket="bucket", Key="foo"
        )
    ...
    async def test_mv_no_overwrite_flag_on_copy_when_small_object_does_not_exist_on_target(
        ...
        assert len(server.requests) == 3, format_requests(server)
        assert_s3_operation(
            server.requests[0], "HeadObject", Bucket="bucket1", Key="key.txt"
        )
        assert_s3_operation(
            server.requests[1],
            "CopyObject",
        ...
        assert_s3_operation(
            server.requests[2], "DeleteObject", Bucket="bucket1", Key="key.txt"
        )
    ...
    async def test_mv_no_overwrite_flag_when_large_object_does_not_exist_on_target(
        ...
        assert len(server.requests) == 7, format_requests(server)
        assert_s3_operation(
            server.requests[0], "HeadObject", Bucket="bucket1", Key="key1.txt"
        )
        assert_s3_operation(
            server.requests[1],
            "GetObjectTagging",
    ...
    async def test_no_overwrite_flag_on_mv_download_when_single_object_exists_at_target(
        ...
        assert len(server.requests) == 1, format_requests(server)
        assert_s3_operation(
            server.requests[0], "HeadObject", Bucket="bucket", Key="foo.txt"
        )
    ...
    async def test_no_overwrite_flag_on_mv_download_when_single_object_does_not_exist_at_target(
        ...
        assert_s3_operation(
            server.requests[1], "GetObject", Bucket="bucket", Key="foo.txt"
        )
        assert_s3_operation(
            server.requests[2], "DeleteObject", Bucket="bucket", Key="foo.txt"
        )
...
class TestMvCommandWithValidateSameS3Paths:
    ...
    async def test_mv_works_if_access_point_arn_resolves_to_different_bucket(
        ...
        assert rc == 0, stderr.decode()
        assert len(server.requests) == 4, format_requests(server)
        assert_operation(server.requests[0], "s3control", "GetAccessPoint")
        assert_s3_operation(
            server.requests[1], "HeadObject", Bucket="bucket", Key="key"
        )
        assert_s3_operation(server.requests[2], "CopyObject")
        assert_s3_operation(
            server.requests[3], "DeleteObject", Bucket="bucket", Key="key"
        )
    ...
    async def test_mv_works_if_access_point_alias_resolves_to_different_bucket(
        ...
        assert rc == 0, stderr.decode()
        assert len(server.requests) == 5, format_requests(server)
        assert_operation(server.requests[0], "sts", "GetCallerIdentity")
        assert_operation(server.requests[1], "s3control", "GetAccessPoint")
        assert_s3_operation(
            server.requests[2], "HeadObject", Bucket="bucket", Key="key"
        )
        assert_s3_operation(server.requests[3], "CopyObject")
        assert_s3_operation(
            server.requests[4], "DeleteObject", Bucket="bucket", Key="key"
        )

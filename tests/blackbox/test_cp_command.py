...
class TestCPCommand:
    async def test_operations_used_in_upload(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(req, "PutObject", Bucket="bucket", Key="key.txt")
    ...
    async def test_key_name_added_when_only_bucket_provided(
        ...
        assert len(server.requests) == 1, format_requests(server)
        req = server.requests[0]
        assert_s3_operation(req, "PutObject", Bucket="bucket", Key="foo.txt")
    ...
    async def test_operations_used_in_download_file(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[1], "GetObject", Bucket="bucket", Key="key.txt"
        )
    ...
    async def test_no_overwrite_flag_when_object_not_exists_on_target(
        ...
        req = server.requests[0]
        assert_s3_operation(
            req, "PutObject", Bucket="bucket", Key="key.txt", IfNoneMatch="*"
        )
    ...
    async def test_recursive_glacier_download_with_force_glacier(
        ...
        assert_s3_operation(
            server.requests[1], "GetObject", Bucket="bucket", Key="foo/bar.txt"
        )
...
class TestAccesspointCPCommand:
    ...
    async def test_upload(self, aws_cli, tmp_path):
        ...
        assert len(server.requests) == 1, format_requests(server)
        assert_s3_operation(
            server.requests[0], "PutObject", Bucket=self.ARN, Key="mykey"
        )
    ...
    async def test_recursive_upload(self, aws_cli, tmp_path):
        ...
        assert len(server.requests) == 1, format_requests(server)
        assert_s3_operation(
            server.requests[0], "PutObject", Bucket=self.ARN, Key="myfile"
        )
    ...
    async def test_download(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[1], "GetObject", Bucket=self.ARN, Key="mykey"
        )
    ...
    async def test_copy(self, aws_cli, tmp_path):
        ...
        assert_s3_operation(
            server.requests[1], "CopyObject", Bucket=dest_arn, Key="mykey"
        )
    ...
    async def test_accepts_mrap_arns(self, aws_cli, tmp_path):
        ...
        assert len(server.requests) == 1, format_requests(server)
        assert_s3_operation(
            server.requests[0], "PutObject", Bucket=mrap_arn, Key="mykey"
        )
...
class TestCpSourceRegion:
    ...
    async def test_respects_source_region_for_recursive_mp_copy(
        ...
        assert_s3_operation(
            server.requests[0], "ListObjectsV2", Bucket=self.SOURCE_BUCKET
        )
        assert server.requests[0].headers.get("host") == self.SOURCE_HOST
